...
class DiscountStoreCubit extends Cubit<DiscountStoreCubitState>{
  ...
  loadDiscountStoreList() async {
    try{
      if(state is LoadingDiscountStoreCubitState || state is ErrorDiscountStoreCubitState){
        return;
      }
      print(state.discountStoreResult.currentPage);
      emit(LoadingDiscountStoreCubitState(discountStoreResult: state.discountStoreResult));
      var result = await _dio.get('/discountStore/list/all', queryParameters: {
        'page': state.discountStoreResult.currentPage,
      });
      await Future.delayed(Duration(milliseconds: 500));
      emit(LoadedDiscountStoreCubitState(discountStoreResult: state.discountStoreResult.copywithFromJson(result.data)));
    } catch (e) {
      emit(ErrorDiscountStoreCubitState(discountStoreResult: state.discountStoreResult, errorMessage: e.toString()));
    }
  }
}
...
class InitDiscountStoreCubitState extends DiscountStoreCubitState{
  InitDiscountStoreCubitState() : super(discountStoreResult: DiscountStoreResult.init());
...
class ErrorDiscountStoreCubitState extends DiscountStoreCubitState{
  String errorMessage;
  ErrorDiscountStoreCubitState({required super.discountStoreResult, required this.errorMessage});
  @override
